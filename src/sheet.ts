...
export class Sheet {
  ...
  private buildProcessedData(): any[][] {
    ...
    const processedRows = this.data.map((rowData, rowIndex) => {
      return this.columns.map((col, colIndex) => {
        ...
        return this.applyPatternStyles(rowData[col.key], context, col.patterns, baseStyle);
      });
    });
  ...
  private applyPatternStyles(
    ...
    const hasPattern = !!patterns;
    if (hasPattern) {
      const bg = applyPattern(patterns.bgColorPattern, context, this.customPatterns);
      if (bg) style = { ...style, ...bg };

      const txt = applyPattern(patterns.textPattern, context, this.customPatterns);
      if (txt) style = { ...style, ...txt };

      const hasCustomPatterns = patterns.custom && patterns.custom.length > 0;
      if (hasCustomPatterns) {
        patterns.custom!.forEach(pattern => {
          const s = applyPattern(pattern, context, this.customPatterns);
          if (s) {
            // Deep merge some properties if needed, but for now simple spread
            style = {
              ...style,
              ...s,
              font: { ...(style.font || {}), ...(s.font || {}) },
